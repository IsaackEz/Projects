...
struct ListaSimple {
	...
	void agregar(int nuevoDato){
		...
		// Hay m�s datos ligados, buscar la �ltima posici�n:
		Nodo *actual = inicio;
		// Hay algui�n despu�s de mi?
		while (actual->next != 0){
			actual = actual->next; // Movernos al siguiente...
			// actual = (*actual).next;
		}

		// Actual en este momento es el �ltimo en la lista
		// ligarle el nuevo dato:
		actual->next = nuevo;
	}

	// Esta funci�n elimina un nodo de la lista, esto basado en el valor que se le provea. 
	// Eliminar� el primer nodo que se encuentre cuyo dato coincida con el dato a eliminar. 
	// Se pueden presentar algunos casos de la lista: 
	// Caso 1: Solamente hay un Nodo en la lista (representar� un nodo con N, el nodo actual con NC, y nodo anterior con NP, 0 para nada): 
	//           NA -> N <- inicio, NP -> 0
	//
	...
	//         el dato a eliminar, suponga que coinciden. Entonces lo que suceder� es que inicio debe moverse al siguiente nodo y eliminar 
	//         de la memoria lo que apunta NC: delete NA, inicio -> 0, NP -> 0
	//         Esto porque hab�a solamente un nodo en la lista. 
	//          

	// Caso 2: Hay m�s de un nodo en la lista y el nodo que contiene el dato a eliminar no est� al principio de la misma. 
	//           N1-> N2 -> N3 -> ... -> Nk-1 -> Nk
	//
	...
	// Esta funci�n elimina todos los nodos de la lista y libera la memoria reservada por cada nodo en la lista:
	void limpiar() 
	{
		Nodo *tmp = 0;
	...
	// Esta funci�n determinar cu�ntos nodos existen en la lista actual:
	int longitud()
	{
	...
	// La siguiente funci�n realiza una b�squeda por un valor dentro la lista (en realidad si est� en alg�n nodo)
	// La funci�n regresa cero o nulo para indicar que el valor no se encuentra en la lista; en otro caso, regresa 
	// la direcci�n de memoria del Nodo que contiene el valor.
	Nodo* buscar(int valor)
	{
		Nodo *tmp = inicio; // Hacemos una variable Nodo temporal para visitar a todos los posibles nodos de la lista

		while (tmp != 0) // Mientras estemos visitando un Nodo v�lido (diferente de Nulo)
		{
		...
		return tmp; // Salimos con la direcci�n de memoria del Nodo que contiene al valor
	}

	// La funci�n insertar intentar� insertar nuevoValor en la lista a partir de un valor que se supone 
	// existe en la lista (despuesDe). Por ejemplo: considere la lista 1->2->45->10; si llamamos la funci�n 
	// insertar(2, -3) entonces deber�amos terminar con la lista 1->2->-3->45->10 porque el valor despuesDe si est�
	// en la lista. Pero si sobre esa misma lista se invoca la funci�n insertar(-1, 4) entonces la lista no cambiar� porque 
	// -1 no est� en la lista.
	void insertar(int despuesDe, int nuevoValor)
	{
		...
		{
			return; // Ignorar porque no podemos insertar a partir de un valor que no est� en la lista.
		}
